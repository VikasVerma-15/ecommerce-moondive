...
export default function HeroSection() {
  ...
  return (
    <section className="section-width px-4 xl:px-0 flex flex-col lg:flex-row gap-5 mb-10 mt-4">
      {/* Sidebar Categories */}
      <div className="w-full lg:w-1/4 lg:border-r border-gray-200 pt-6 lg:pr-6 hidden sm:flex flex-col gap-4">
        {categories.map((category, index) => {
          const slug = category.slug;
          return (
            <Link href={`/products?category=${slug}`} key={index} className="flex items-center justify-between group cursor-pointer">
              <span className="text-gray-900 font-medium hover:text-[#DB4444] transition-colors">
                {category.name}
              </span>
              {category.hasSub && (
                <ChevronRight size={20} className="text-gray-900 group-hover:text-[#DB4444]" />
              )}
            </Link>
          );
        })}
      ...
      <div className="w-full lg:w-3/4 pt-6">
        <div className="bg-black w-full h-[350px] sm:h-[400px] lg:h-[400px] relative overflow-hidden flex flex-col md:flex-row">
          ...
          >
            {slides.map((slide, idx) => (
              <div key={slide.id} className="w-full h-full flex flex-col md:flex-row shrink-0">
                {/* Text Content */}
                <div className="flex flex-col justify-center px-10 md:px-16 z-10 pt-10 md:pt-0 w-full md:w-1/2">
                  ...
                  <h1 className="text-white text-4xl md:text-5xl lg:text-6xl font-semibold leading-tight mb-6 whitespace-pre-line">
                    {slide.title}
                  </h1>
                  
                  <Link 
                    href="/products?category=phone" 
                    className="flex items-center gap-2 text-white font-medium hover:text-gray-300 transition-colors border-b border-white w-max pb-1"
                  >
